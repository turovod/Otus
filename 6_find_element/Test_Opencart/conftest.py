...
def initial_test(request):
    if request.config.getoption('--brouser') == 'C':
        driver = webdriver.Chrome(executable_path='/5_Test_Opencart/drivers/chromedriver')
        driver.get('http://localhost:8888/opencart3/upload/')
        driver.fullscreen_window()

    if request.config.getoption('--brouser') == 'F':
        driver = webdriver.Firefox(executable_path='/5_Test_Opencart/drivers/geckodriver')
        driver.get('http://localhost:8888/opencart3/upload/')
